def test_create_student(client):
    ...
    payload = {"reg_no": "2312254", "name": "Test Student", "course": "DevOps"}
    response = client.post("/students", json=payload)
    assert response.status_code == 201
    data = response.json()
    assert data["reg_no"] == "2312254"
...
def test_get_students_after_create(client):
    ...
    payload = {"reg_no": "2312556", "name": "Other Student", "course": "Cloud"}
...
def test_get_student_by_reg_no(client):
    ...
    payload = {"reg_no": "2312228", "name": "Another Student", "course": "AWS"}
    client.post("/students", json=payload)
    response = client.get("/students/2312228")
    assert response.status_code == 200
    assert response.json()["name"] == "Another Student"
